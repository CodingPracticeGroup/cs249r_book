...
_为什么构建 ML systems 所需的 engineering principles 与 governing 传统 computing systems 的原则如此不同？_

ML systems 有其 physics：data 在 bandwidth 约束的 memory hierarchies 中流动，arithmetic 在 power 约束的 silicon 上运行，predictions 必须在 latency windows 内到达。这些 constraints 不是 implementation details；它们塑造了从 model architecture 到 deployment target 的每一个 decision。ML systems 还不同于传统 computing systems，因为 behavior 是由 data 定义的，而不仅仅是 explicit logic 或 hardware state。当一个 conventional program 出现异常时，engineers 通常可以 trace source 或 inspect hardware state；当一个 ML system 出现异常时，code 可能执行正确，但 learned behavior 失败了，因为 data 不完整、有 bias、过时或不再 representative。ML engineers 因此需要同时管理 statistical uncertainty 和 physical execution constraints。一个在 data center 中 fitting 的 model 在 phone 上可能毫无用处；一个 training pipeline 在一个 accelerator 上一周就能 converge，在另一个上可能需要一个月；用去年 data 训练的 accurate model 可能会 silently degrade。Traditional practices 如 testing、modularity、version control 和 performance analysis 仍然是必要的，但不够。本卷建立一门 grounded in computation's physical limits 的 discipline：algorithmic choices 影响 stack 一直到 machine，hardware constraints 又反向影响 model design。本卷每个 part 开篇列出该 stage 的 active principles：foundations、development、optimization 和 deployment。Later chapters 只在读者有 context 使用这些 principles 后才回归，这样 new techniques 作为 growing constraint set 的 instances 出现，而不是作为 isolated tools。
...
- 解释为什么 data-defined behavior 和 physical constraints 使 ML systems 区别于 traditional software
- 应用 Data-Algorithm-Machine lens 诊断 data movement、arithmetic 和 machine limits 的 bottlenecks
- 通过 bitter lesson 分析 AI 从 symbolic rules 到 deep learning 的 shift
- 计算 iron-law performance terms 推理 throughput、latency 和 return on compute
- 将 lifecycle、deployment、degradation 和 five-pillar perspectives 综合为 ML systems engineering judgments
...
ML systems 进入日常生活的方式不是作为 ordinary programs，而是作为在 physical constraint 下运行的 data-shaped behavior。当用户在 smartphone 上提问时，AI system 将 speech 转为 text、interpret intent 并 generate response。浏览 social media 时，AI systems 决定哪些 posts 出现以及以什么顺序。申请 loan 时，AI systems 评估 creditworthiness。驾驶 modern car 时，AI systems 监控 lane position、detect pedestrians 并 adjust cruise control。每种情况下，system 不只是 retrieving information，而是在 uncertainty 下 making decisions，常常 controlling physical outcomes 影响 safety、finances 或 access to opportunity。这些不是 future possibilities；它们是 present realities，每天影响着 billions 的人。

构建这些 systems 成为区别于 traditional software 的 engineering challenge，因为 **dual mandate**\index{Dual Mandate}。每个 ML system 必须同时管理 statistical uncertainty——因为 model 的 predictions 是 probabilistic 的——以及 physical constraints——因为 executing those predictions 需要 moving terabytes of data 和 performing quintillions of arithmetic operations，常常在 milliseconds 内。在 failure boundaries 处区别最明显：a code bug causes a crash，a loud failure；a data bug causes a wrong prediction，a silent one\index{Silent Degradation}。当 ML system 的 accuracy 下降 5 个百分点时，training data 可能 shifted、hardware 可能在 mid-training 耗尽 memory，或 model 可能没有 converged。当 system behavior 由 data 而非 code 定义时，debugging、testing 和 architectural design 全部改变。

这个 dual mandate 在 every large-scale AI deployment 中可见。Conversational AI services 协调 data centers 中 large pools of GPUs[^fn-gpu-parallel]，per query executing enormous numbers of operations，同时 managing memory、network bandwidth 和 thermal constraints。Modern driver-assistance 和 autonomous-driving systems 处理 high-rate sensor streams，常常 combining cameras with radar、LiDAR 或 other sensors depending on the vehicle platform，并在 milliseconds 内 fuse perception into control decisions。Google 每天处理 `{python} AIMomentStats.google_search_b_str` 次 searches，每次 triggering multiple AI systems for ranking、knowledge extraction 和 spell-checking，同时 meeting strict latency targets on globally distributed infrastructure。这些 systems 不只是 running algorithms。它们在 tight physical constraints 下 orchestrate data、computation 和 hardware，以 deliver statistically reliable results at scale。在它们下面，dual mandate 的 deeper implication：当 data 而非 code 定义 behavior 时，software 的本质改变了。
...
当一个 traditional program 失败时，engineer 通常可以 trace a branch、inspect a stack frame 并 patch the code path。当 ML system 的 accuracy 在没有 code change 的情况下下降时，debugging target 可能是 shifted data distribution、changed label process 或 no longer represents production behavior 的 model。Andrej Karpathy[^fn-karpathy-sw2] 将这一 distinction 形式化为从 **Software 1.0**\index{Software 1.0} 到 **Software 2.0**\index{Software 2.0} 的 shift [@karpathy2017software]，这是 programming-model shift 从 hand-written logic 到 learned weights 的 framing。@Tbl-software-1-vs-2 term by term 映射了这一 shift，而 driving the rest of this chapter 的 row 是 systems consequence：Software 1.0 fails loudly with a crash，而 Software 2.0 可以通过 metric degradation silently degrade，failure 保持 invisible 直到 monitoring system catches it。
...
: **从 Software 1.0 到 Software 2.0 的 Paradigm Shift**: In Software 2.0, the "programmer" does not write the logic; they curate the dataset that the optimization process uses to write the logic. Debugging therefore moves upstream from code to data. The "compiler" analogy is approximate: unlike a deterministic compiler, the training process is stochastic and may produce different "executables" from the same "source code." {#tbl-software-1-vs-2}

The data-centered workflow creates a systems cost that does not appear in ordinary software projects: model behavior depends on pipelines, labels, monitoring, and feedback loops that surround the learned code. Google researchers quantified that hidden technical debt\index{Technical Debt} in a landmark 2015 paper.

::: {.column-margin}
![](images/svg/vol1_introduction_margin_001.svg){width="100%" fig-alt="A small orange box labeled ML code 5 percent nested inside a much larger surrounding frame labeled System 95 percent; the inner box occupies a tiny fraction of the outer area."}
...
**Context**: Google Flu Trends (GFT) estimated influenza activity from aggregated search-query patterns, and was held up as the canonical example of a data-rich predictive system built on behavioral traces rather than direct measurement [@ginsberg2009detecting]. In a 2014 *Science* paper, David Lazer and colleagues at Northeastern, Harvard, and the University of Houston audited GFT against CDC ground truth across its 2011--2013 operating window [@lazer2014].
...
[^fn-model-weights-inference]: **Model Weights**: The learned numerical parameters of a neural network, one value per connection between units. A GPT-3-scale model stores `{python} GPT3WeightFootprint.params_b_str` such values, consuming `{python} GPT3WeightFootprint.fp16_weights_gb_str` in FP16 precision, a 16-bit floating-point format that uses two bytes per value [@brown2020language]. Because every inference request must load these weights through the memory hierarchy, weight count is the single largest determinant of both memory footprint and serving cost (see @sec-neural-computation). \index{Model Weights!definition}

从 systems perspective 来看，这代表了从 *instruction-centric* 到 *data-centric* computing 的 transition [@ng2021datacentric] \index{Data-Centric Computing}。在传统的 *instruction-centric*\index{Instruction-Centric Computing!traditional paradigm} model 中，systems 被优化为高效执行 hand-crafted logic，programmer 的工作是写正确的 instructions。在 ML 的 *data-centric* model 中，systems 被优化为高效 ingestion of data 和 iterative refinement of model parameters，programmer 的工作是 curate correct data。

因此，debugging an ML system 意味着 debugging the data，而不是 debugging Python scripts。Version control 必须 track datasets，而不仅仅是 git commits。Testing 必须 validate data distributions，而不仅仅是 code paths。然而，即使 thorough testing 也无法 closing what amounts to a structural **verification gap**\index{Verification Gap} between finite test sets and the vast continuous input spaces that ML systems encounter in production。
...
class VerificationGap:
    ...
    test_size = Datasets.ImageNet.test_examples.to(count).magnitude
...
The verification gap is symptomatic of a deeper shift: from *deterministic* systems where correctness can be proven to *probabilistic* systems where it can only be bounded\index{Probabilistic Engineering}。经典 systems engineering 中，success 由 *determinism* 定义：same input always yields same output。AI engineering 中，variance is inherent；data 的"squishiness"（noise、drift、hidden patterns）是 system intelligence 的 source，也是 unpredictability 的 source。Traditional systems 通过 *resistance* to change 实现 robustness，ML systems 通过 *adaptation* to change 实现 robustness。因此，AI 中真正的 robustness 来自 engineering *observability* 和 *adaptation*，而非 *rigidity*。

Rethinking the stack 也需要 historical context。从 instruction-centric 到 data-centric computing 的 shift 并非 overnight；它通过 seven decades of paradigm transitions 逐渐 emerge，each overcoming the bottlenecks of its predecessor。Each era of AI faced a characteristic bottleneck，understanding those bottlenecks reveals *why* systems engineering became central to progress。

::: {#chk-introduction-paradigm-shift .callout-checkpoint title="The paradigm shift"}

在 tracing the history of AI 之前，verify your understanding of the paradigm shift in how we build software：
...
<!--
TODO: 翻译剩余章节 (原文 lines 474-3283)
以下部分暂保留英文原文，需逐字翻译为简体中文。
翻译约定：正文用中文，专业术语保留英文 (pipeline, inference, quantization 等)。
-->
...
The first era of AI engineering (1950s–1970s) attempted to reduce intelligence to **Symbolic AI**\index{Symbolic AI} manipulation, an approach later crystallized in the physical-symbol-system hypothesis [@newell1976computer]. Researchers at the 1956 **Dartmouth Conference**\index{Dartmouth Conference}[^fn-dartmouth-systems] [@mccarthy1955dartmouth] hypothesized that aspects of intelligence could be precisely described and simulated by machines. Even then, some saw a different path: Arthur Samuel at IBM demonstrated in 1959 that a checkers program could improve through self-play, coining the very term "**machine learning**\index{Machine Learning}" [@samuel1959], though the dominant paradigm remained symbolic. Daniel Bobrow's STUDENT[^fn-bobrow-student] system exemplifies this approach [@bobrow1964student].
...
In the expert-systems era\index{Expert Systems}, engineers pivoted from general logic to capturing deep domain expertise. MYCIN\index{MYCIN!expert system}, designed to diagnose blood infections, encoded medical knowledge as IF-THEN production rules and included rule-acquisition capabilities for expert input [@shortliffe1975].
...
MYCIN outperformed junior doctors in specific tests but revealed the **knowledge acquisition bottleneck**\index{Knowledge Acquisition Bottleneck}[^fn-knowledge-bottleneck-scale]. Extracting implicit intuition from human experts and formalizing it into IF-THEN rules proved slow, error prone, and contradictory.

[^fn-knowledge-bottleneck-scale]: **Knowledge Acquisition Bottleneck**: Feigenbaum's knowledge-engineering work framed applied AI around the practical difficulty of extracting, representing, and maintaining expert knowledge [@feigenbaum1984knowledge]. In systems terms, this bottleneck was a throughput problem: knowledge elicitation and rule maintenance were bound by the serial bandwidth of human experts. Unlike computational bottlenecks that yield to faster hardware, this one was the original "does not scale" constraint in AI and a direct motivation for the data-driven paradigm that followed.
...
[^fn-viola-jones-earlyexit]: **Viola-Jones Algorithm**: The algorithm's real-time speed came from a classifier cascade that used simple, hand-engineered rectangular features to immediately reject nonface regions. The method was designed and evaluated for frontal-face detection, illustrating the era's trade-off: expert feature design could be fast and effective, but the representation was task-specific. The first two layers alone could discard over 80 percent of negative sub-windows while using just eleven of the 6,000+ total features [@viola2001]. \index{Viola-Jones Algorithm!compute efficiency}
...
from mlsysim.core.units import Mparam, param
...
Deep learning\index{Deep Learning} removed the human feature engineering requirement. Neural networks learn representations directly from raw data (pixels, audio waveforms), enabling "end-to-end" learning. This shift was not simply a new-algorithm story: CNNs existed in earlier forms [@lecun1998; @lecun2015], while AlexNet combined architectural and training choices with **systems co-design**\index{Systems Co-design}: choosing model structure, training procedure, and hardware mapping together rather than treating hardware as an afterthought. The AlexNet breakthrough\index{AlexNet}\index{ImageNet} [@krizhevsky2012imagenet] occurred because algorithmic structure (parallel matrix operations) matched hardware capabilities (GPUs). With `{python} AlexNetBreakthrough.alexnet_params_m_str` parameters distributed across two GTX 580 GPUs, AlexNet achieved 15.3 percent top-5 error, a `{python} AlexNetBreakthrough.alexnet_relative_improvement_str` relative improvement over the next-best entry that year, through both model choices and hardware-algorithm alignment. @Fig-alexnet makes this co-design visible. The labeled boxes are the network's successive processing stages, the convolution and pooling layers that extract image features and the fully connected layers that produce the final classification, all developed in later chapters; what matters here is that the architecture splits into two parallel processing streams. That split reflected the memory limits of a single GTX 580 GPU, making part of the network's structure a product of its hardware constraints.
...
Deep learning effectively traded the feature engineering bottleneck for a new **compute bottleneck**\index{Compute Bottleneck}. Models like GPT-3 [@brown2020language] (`{python} GPT3Scale.gpt3_params_billion_str` parameters) illustrate the scale of this new challenge. Brown et al. report training on about `{python} GPT3Scale.gpt3_training_tokens_billion_str` tokens from filtered web text, books, and Wikipedia. Using the book's dense-training approximation, that parameter-token scale implies roughly `{python} GPT3Scale.gpt3_training_zflops_str` zettaFLOPs of compute; because the GPT-3 paper does not specify the exact hardware configuration, any V100 GPU-year conversion is an illustrative internal estimate rather than a reported fact. (One zettaFLOP equals $10^{21}$ floating-point operations; the training corpus comprised roughly `{python} GPT3Scale.data_scale_gb_str` of text.) The primary engineering challenge shifted from "*how* do we describe a cat's ear?" to "*how* do we coordinate large-scale distributed training without failure?"

With these four paradigm shifts traced, the pattern becomes visible in @tbl-ai-evolution-strengths: each era's breakthrough came not from cleverer algorithms but from removing a systems bottleneck that prevented existing algorithms from using more data and computation. Symbolic AI had the algorithms for logic but lacked the data; expert systems had domain knowledge but could not scale it; statistical learning had the data but required human feature engineering; deep learning automated feature learning but demanded infrastructure that did not yet exist. The recurring theme is that *systems innovations*, not *algorithmic innovations*, enabled each transition, and it raises a practical dilemma: given limited resources, organizations must decide whether to invest in better algorithms, larger datasets, or higher-throughput hardware. One of AI's leading researchers examined the historical record systematically and reached a conclusion that challenges our deepest intuitions about how intelligence should be built.
...
class GPT4ScaleEstimate:
    ...
    check(0.8 <= estimate_ratio <= 1.1, "Illustrative 25k reference-GPU x 90 day estimate should be near the mlsysim anchor.")
...
@Tbl-ai-evolution-performance quantifies the shift from expert systems to statistical learning to deep learning: representative task performance improved as each transition unlocked more computational scale rather than more elaborate encodings of human knowledge. The rightmost column shows the systems pattern: as computational resources grew from rule evaluation to CPU-era feature pipelines, multi-GPU training, and eventually large-scale distributed training, performance improved from amateur-level to superhuman. GPT-4's exact training configuration was not disclosed, so the table uses an illustrative `mlsysim` reference anchor rather than an official training disclosure: `{python} GPT4ScaleEstimate.a100_gpu_days_million_str`, the same order of magnitude as `{python} GPT4ScaleEstimate.a100_class_gpu_count_str` reference GPUs for roughly `{python} GPT4ScaleEstimate.training_days_str` [@semianalysisGPT4].
...
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, fmt_count, fmt_time
...
This definition motivates the **Data · Algorithm · Machine (D·A·M) taxonomy**\index{D·A·M Taxonomy}, which we now formalize as a diagnostic tool: when performance stalls or behavior degrades, the first diagnostic step is to identify the binding axis.

::: {#dfn-introduction-d-m-taxonomy .callout-definition title="The D·A·M taxonomy"}

***D·A·M Taxonomy***\index{D·A·M Taxonomy!definition} is a diagnostic framework that classifies any machine learning system performance bottleneck along three axes: Data\index{Data!as source code}, which determines what examples and bytes the system must process; Algorithm, which determines the model structure and work required to learn or predict; and Machine, which determines the hardware capacity available to execute that work. The goal is to identify which axis is the binding constraint.
...
The three components can be conceptualized as Data (the fuel), Algorithm (the blueprint), and Machine (the engine). Without any one component, the others remain theoretical. The D·A·M taxonomy captures this interdependence directly; @fig-ai-triad shows why the three elements cannot be designed, or even reasoned about, in isolation.
...
: **The D·A·M Taxonomy**\index{D·A·M Taxonomy!components}: Every ML system comprises these three interdependent axes, and optimizing one in isolation typically shifts the bottleneck to another rather than eliminating it. The recurring diagnostic step throughout this text is to locate which axis is binding before choosing an optimization. {#tbl-dam-taxonomy}
...
### From silicon to mission: A four-layer hierarchy {#sec-introduction-engineering-crux}
...
1.  **Hardware (The Silicon)**: The physical foundation (The Engine). This layer defines the raw capabilities: peak compute throughput $(R_{\text{peak}})$, memory bandwidth $(\text{BW})$, and memory capacity; concrete hardware twins instantiate those quantities when deployment scenarios need numeric constraints.
...
3.  **Workloads (The Models)**: The algorithmic demand (The Route). This layer defines the mathematical workload: operation count $(O)$, data volume moved $(D_{\text{vol}})$, and data layout. Scenario-specific workloads, such as GPT-4 and Wake Vision\index{Wake Vision!smart doorbell}, instantiate these demands for particular missions.
...
The multi-order-of-magnitude span across these four paradigms is not merely a technical curiosity; it translates directly into cost. A model that fits comfortably in a data-center accelerator's memory cannot run unchanged on a microcontroller-class device, and bridging that gap requires engineering trade-offs at every tier of the D·A·M taxonomy. Data quality, algorithmic efficiency, and hardware capability interact through **samples per dollar**\index{Samples per Dollar}, a single economic constraint that systems engineers must optimize.
...
The D·A·M taxonomy reveals *what* ML systems comprise: data that guides behavior, algorithms that extract patterns, and machines that enable learning and inference\index{Inference}[^fn-inference-deployment]. The critical distinction between ML systems engineering and traditional software engineering lies not in these components themselves but in *how the resulting systems fail*.
...
3. **Monitor and respond to drift** ($\mathcal{D}(P_t \lVert P_0)$): Continuous measurement of distribution divergence enables proactive retraining before accuracy falls below acceptable thresholds.
...
A training job stalls when storage cannot feed an accelerator; an inference path misses its deadline when model state moves too slowly through memory or across the network. These failures look different, but they share a single performance structure. Machine learning system performance is governed by the **Iron Law of ML Systems**\index{Iron Law of ML Systems!definition}\index{Iron Law of ML Systems!equation}, formalized in @eq-intro-iron-law:
...
3.  **The latency term** $(L_{\text{lat}})$: The irreducible "tax" of system orchestration, networking, and serialization. This fixed latency dominates in real-time deployment. We address this in Part IV: Deploy.
...
We retain "iron law" because, like Amdahl's Law\index{Amdahl's Law!diagnostic analogy} [@amdahl1967], its value lies in diagnostic power: identifying which physical constraint dominates before optimizing. The iron law is useful precisely because it simplifies the complexity of the full stack into three manageable terms. @Sec-appdx-dam-taxonomy presents the refined treatment, including pipelining and overlap techniques that transform the additive model into the max-based formulation used in practice.

:::

As George Box famously said, "All models are wrong, but some are useful" [@box1976science].[^fn-box-ironlaw]
...
class GPT3Training:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Text strings
    accelerator_name_str = MarkdownStr("reference")
    num_accelerators_str = fmt_count(num_accelerators, commas=False, label="accelerator")
...
**Problem**: What is the training time for a GPT-3 class model on a cluster of `{python} GPT3Training.num_accelerators_str` built from `{python} GPT3Training.accelerator_name_str` accelerators?
...
**Systems insight**: If we improve hardware utilization $(\eta_{\text{hw}})$ from `{python} GPT3Training.eta_base_pct_str` to `{python} GPT3Training.eta_opt_pct_str` through better scheduling and more efficient execution, training time drops to `{python} GPT3Training.days_optimized_str`, saving nearly `{python} GPT3Training.days_saved_str` of expensive compute time.
...
::: {.column-margin}
![](images/svg/introduction_energy_hierarchy.svg){width="100%" fig-alt="Vertical ladder of three orange bars on a log scale, longest at top: DRAM 160 pJ towers far above FP16 1.1 pJ and INT8 0.2 pJ, showing data movement costs orders of magnitude more energy than arithmetic."}
...
The decomposition of time is also an economic constraint. In the Hennessy & Patterson tradition of quantitative reasoning, the **return on compute (RoC)**\index{Return on Compute!economic invariant} is defined as the marginal accuracy gain per dollar of infrastructure investment.
...
| **Keyword Spotting**\index{Keyword Spotting!power envelope} | Power envelope                 | Tiny memory and energy budgets     | Can I run always-on inference on milliwatts?   |
...
The same diagnostic reading applies retrospectively to the breakthrough that launched the deep learning era. The AlexNet victory traced in @sec-introduction-deep-learning-era-infrastructure-bottleneck-2f02 was a D·A·M alignment: the error reduction came not from algorithmic novelty alone but from convolutional networks' parallel matrix operations matching GPU capabilities, trained on ImageNet's[^fn-imagenet-scale] [@deng2009] unprecedented labeled corpus.

[^fn-imagenet-scale]: **ImageNet**: The 2009 dataset that proved data scale was the missing ingredient in computer vision. Fei-Fei Li marshaled 49,000 Mechanical Turk workers to label 14.2 million images across 21,841 categories; the 2012 challenge training split used by AlexNet contained about `{python} ImageNetStats.images_m_str` labeled images. This data engineering operation dwarfed the algorithmic novelty of everything it subsequently enabled, including AlexNet's 2012 breakthrough (see @sec-data-engineering). \index{ImageNet!scale}
...
Efficiency is a bottleneck diagnosis, not a single technique. Three complementary dimensions map directly to our D·A·M taxonomy (@tbl-dam-taxonomy), and each changes a different term in the resource budget. Algorithmic efficiency\index{Algorithmic Efficiency!definition}, the earliest frontier, reduces computational requirements through better model design and training procedures. Its goal is to produce more useful behavior per operation, so capability rises without scaling every resource in lockstep. As algorithms demanded ever more computation, compute efficiency\index{Compute Efficiency!hardware utilization} became the second critical dimension. It maximizes hardware utilization by aligning algorithmic logic with machine physics, turning theoretical processor capability into useful work. Most recently, data selection\index{Data Selection!efficiency dimension} emerged as the third dimension, extracting more learning signal from limited examples and thereby reducing the total operations term $O$ of the iron law. The timeline in @fig-evolution-efficiency places these three dimensions on the same axis so the historical sequence of bottlenecks is visible before the chapter sequence presents them in build order. Together, these three dimensions provide the engineering tools to overcome the data, algorithm, and machine walls that pure scaling alone cannot address.
...
class EfficiencyGainsMetrics:
    ...
    moores_window_speedup_mult_str = fmt_multiple(moores_window_speedup, precision=1, commas=False)
    growth_gap_ratio_mult_str = fmt_multiple(growth_gap_ratio, precision=1, commas=False)
```

The architecture-level trajectory in @fig-algo-efficiency makes the algorithmic side of this gap visible model by model, rather than only as an aggregate scaling claim.

::: {#fig-algo-efficiency fig-env="figure" fig-pos="htb" fig-cap="**Algorithmic Efficiency Trajectory**: Training efficiency factor relative to AlexNet (2012 baseline) for ImageNet classification. Most later architectures achieve comparable accuracy with fewer computational resources, although individual points vary. The trajectory from AlexNet (1$\times$) through VGG, ResNet, MobileNet, and ShuffleNet to EfficientNet (44$\times$) demonstrates a forty-four-fold reduction in required compute over seven years, independent of hardware improvements [@hernandez2020measuring]. Early-2020s points are included as author-added context rather than as evidence for extending the Hernandez-Brown trend." fig-alt="Scatter plot of training efficiency factors from AlexNet in 2012 to ConvNeXt in 2022. Red dots rise from AlexNet at 1x to early-2020s models above 80x. A dashed exponential trend line fit through all points curves steeply upward."}
...
[^fn-moores-law-baseline]: **Moore’s Law**: Gordon Moore’s 1965 observation described rapid growth in the number of components that could be economically integrated on a chip [@moore1998]; later industry summaries often expressed the cadence as roughly a two-year doubling. Over the same 2012–2019 window, a two-year doubling cadence yields roughly `{python} EfficiencyGainsMetrics.moores_window_speedup_mult_str` in transistor scaling, meaning algorithmic innovation closed more of the efficiency gap than hardware alone. Simultaneously, demand for training compute doubled every 3.4 months, about `{python} EfficiencyGainsMetrics.growth_gap_ratio_mult_str` faster than the two-year hardware cadence, forcing the shift to domain-specific accelerators. \index{Moore's Law!vs. AI growth}
...
These measurements emerge from rigorous empirical methodology that tracked training compute across hundreds of published models; @sec-benchmarking develops the measurement frameworks that enable such systematic analysis of ML system performance. The two cadences just compared define the systems gap: the widening distance between what models demand (compute doubling every 3.4 months) and what hardware supplies (transistor density doubling roughly every two years). Closing that gap is the primary objective of this textbook, requiring integrated expertise across the software and hardware stack; @sec-hardware-acceleration quantifies it directly.

@Fig-algo-efficiency traces this trajectory architecture by architecture: VGG, ResNet, MobileNet, and EfficientNet each reach comparable accuracy with progressively fewer computational resources. Later ImageNet architectures are shown to give visual context for the mature benchmark, but the cited empirical efficiency claim is the 2012–2019 trajectory.

Efficiency gains tell only half the story. @Fig-ai-training-compute-growth compares illustrative endpoint estimates for AlexNet-era and GPT-4-class training—one visualization of scale, distinct from the aggregate 3.4-month doubling trend cited above—yet the span still grows exponentially, making efficiency optimization not a luxury but a necessity for continued progress.
...
1.  **Significance**: ML research typically optimizes only the algorithm axis ($O$ and convergence). AI engineering jointly optimizes all three: it bounds $D_{\text{vol}}$ by data governance requirements, bounds $O/(R_{\text{peak}} \cdot \eta_{\text{hw}})$ by production latency requirements, and bounds total power draw by energy and cost budgets. A production system that achieves 95 percent accuracy in research but violates a 100 ms latency requirement in production is a failed system, regardless of its algorithm score.
...
The data-dependent nature of ML systems creates dynamic lifecycles requiring continuous monitoring and adaptation. Unlike source code that changes only through developer modifications, data reflects real-world dynamics: distribution shifts can silently alter system behavior without any code changes. The tooling gap identified above follows the system into production: version control built for discrete code changes struggles with large, evolving datasets, and testing frameworks built for deterministic outputs require adaptation for probabilistic predictions. We address data versioning and quality management in @sec-data-engineering and monitoring approaches that handle probabilistic behaviors in @sec-ml-operations.
...
The top of the hierarchy transforms abstract systems into concrete engineering missions. Each mission inherits one of the four deployment paradigms introduced in @sec-introduction-engineering-crux and pairs it with a scenario-specific workload. @Tbl-introduction-engineering-missions makes the pairing concrete for the four application scenarios\index{Application Scenarios!definition} that recur throughout the book and its associated labs. These missions act as the end-to-end verification for our engineering decisions. A 2$\times$ increase in memory bandwidth is an academic result until it is proven to extend the battery life of the Smart Doorbell or enable the safety-critical latency required for Autonomous Perception.

:::

| **Mission**                                                          | **Deployment Paradigm** | **Scenario Workload**      | **Critical Constraint**                                  |
|:---------------------------------------------------------------------|:------------------------|:---------------------------|:---------------------------------------------------------|
| **Frontier Training**\index{Frontier Training!cloud cluster}         | Cloud Cluster           | GPT-4                      | Target: `{python} ScenarioRegistry.frontier_limit_str`   |
| **Autonomous Perception**\index{Autonomous Perception!edge robotics} | Edge Robotics           | YOLOv8-nano                | SLA: `{python} ScenarioRegistry.drive_limit_str` latency |
| **Mobile Assistant**\index{Mobile Assistant!smartphone constraints}  | Smartphone              | Mobile-optimized small LLM | Thermal Throttling/RAM                                   |
| **Smart Doorbell**                                                   | TinyML (MCU)            | Wake Vision                | Power: `{python} ScenarioRegistry.doorbell_power_str`    |

: **Four Engineering Missions**: Pairs each deployment paradigm with a scenario workload and its binding constraint, mapping the book's running application scenarios onto the cloud/edge/mobile/TinyML hierarchy. {#tbl-introduction-engineering-missions tbl-colwidths="[25,25,25,25]"}

Each mission runs this lifecycle continuously, and because the stages feed one another, the loop compounds in whichever direction it is pushed: high-quality data improves the model, which improves the product, which collects better data, while a single weak stage drags every downstream stage with it. The wake-word failure chain examined in @sec-introduction-five-engineering-disciplines-fa08 traces one such vicious cycle stage by stage.
...
[^fn-microcontrollers-tinyml]: [offset=-42mm] **Microcontrollers**: Single-chip computers with kilobytes of memory and milliwatts of power budget. For TinyML, memory and energy determine feasibility before model quality. \index{Microcontroller!TinyML substrate}

Between these poles, placement becomes a constraint-allocation problem. Edge ML systems\index{Edge ML!latency and bandwidth} bring computation closer to data sources, reducing latency[^fn-latency-responsiveness] and bandwidth requirements while managing local computing resources. Mobile ML systems\index{Mobile ML!resource constraints} must balance sophisticated capabilities against memory shared with every other application, processors throttled by thermal limits, and a battery budget the whole device must ration; an on-device model operates orders of magnitude below a data-center server's power draw, trading raw speed for locality and privacy (@sec-ml-systems quantifies the per-tier envelopes). Enterprise ML systems often operate within specific business constraints, focusing on particular tasks while integrating with existing infrastructure. Some organizations employ hybrid approaches, distributing ML capabilities across multiple tiers to balance latency, privacy, bandwidth, and update control.

[^fn-latency-responsiveness]: [offset=-42mm] **Latency**\index{Latency!etymology}\index{Latency!edge deployment}: From Latin *latere* ("to lie hidden"), delay is invisible until it causes failure. In autonomous braking, every millisecond adds roughly 3 cm of stopping distance, making $L_{\text{lat}}$ the edge constraint.
...
- **Waymo**\index{Deployment!high-stakes hybrid deployment, autonomous vehicles}[^fn-waymo-hybrid] [@sun2020waymo] binds on safety-critical latency and data freshness. Its autonomous-driving perception stack illustrates a *high-stakes hybrid* deployment pattern: on-vehicle perception models run at the edge under strict latency requirements, while cloud infrastructure supports training and evaluation on large-scale multimodal driving data.
- **FarmBeats**\index{Deployment!resource-constrained edge, precision agriculture}[^fn-farmbeats-edge] [@vasisht2017farmbeats] binds on connectivity and model freshness. Microsoft's precision agriculture platform deploys ML models to farms with limited connectivity. FarmBeats represents the *resource-constrained edge* deployment pattern: compact models run inference on low-power devices while network links constrain how quickly data and updates can move.
- **AlphaFold**\index{Deployment!compute-intensive cloud deployment, scientific computing}\index{AlphaFold!cloud deployment} [@jumper2021] binds on compute-intensive search and curated scientific data. DeepMind's protein structure prediction system solved a 50-year grand challenge in biology. AlphaFold represents the *compute-intensive cloud* deployment pattern: training required 128 TPUv3 cores for weeks and drew on the Protein Data Bank's experimentally determined structures.

[^fn-waymo-hybrid]: **Waymo**: A high-stakes hybrid autonomous-driving workflow forces a synchronization challenge absent from pure cloud or pure edge systems: the on-vehicle model must be controlled and regression-tested before deployment, while cloud infrastructure can train and evaluate improved versions on newly collected driving data. This creates a version-management gap between deployed and newly trained models, requiring rigorous validation before any remote model update can be pushed to safety-critical vehicles. \index{Waymo!hybrid deployment}

[^fn-farmbeats-edge]: **FarmBeats**: The system demonstrates that the binding constraint for edge ML is often network $\text{BW}$ rather than compute or model quality. By using TV white-space networking and edge processing, FarmBeats makes model freshness and data synchronization first-class engineering problems in connectivity-constrained deployments [@vasisht2017farmbeats]. \index{FarmBeats!connectivity constraint}
...
# │ Show: The order-of-magnitude scale of real-world multimodal data ingestion.
# │ How: List illustrative low and high sensor data-rate anchors.
# │
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_qty, check
from mlsysim.core.units import TB, hour
from mlsysim import ReferenceStats
...
class WaymoStats:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    # Waymo data-rate range (ReferenceStats registry)
    rate_low_raw = ReferenceStats.Workloads.WaymoDataPerHourLow
    rate_high_raw = ReferenceStats.Workloads.WaymoDataPerHourHigh

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    val_low = rate_low_raw
    val_high = rate_high_raw

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(val_low.to("TB/hour").magnitude >= 1, f"Waymo data rate ({val_low.to('TB/hour').magnitude} TB/hr) is too low.")
    check(val_high.to("TB/hour").magnitude > val_low.to("TB/hour").magnitude, "High rate must be > Low rate.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
```

Real-world data is often noisy and inconsistent, presenting the first category of challenges. Waymo's autonomous vehicles serve as roving data centers, processing large multimodal sensor streams across LiDAR[^fn-lidar-pointcloud], radar[^fn-radar-robustness], and cameras [@sun2020waymo]. Engineers must solve for sensor interference, such as rain obscuring cameras, and temporal misalignment across asynchronous data streams. Scale compounds these quality issues: FarmBeats sits at one extreme, its sub-megabyte models squeezed through the kilobit-class links described earlier, while AlphaFold occupies the opposite, requiring access to the Protein Data Bank's experimentally determined structures during training.

Data drift\index{Data Drift!operational burden} creates an ongoing operational burden atop both quality and scale. The statistical properties of input data change over time, and models are only as reliable as their alignment with the current distribution [@gama2014; @quinonero2009dataset; @koh2021wilds]. Waymo models trained on Phoenix's sun-drenched roads may fail in New York's snowstorms due to distribution shift[^fn-drift-entropy]; detecting these shifts requires continuous monitoring of input statistics before they manifest as system failures.
...
This book covers the **single-node**\index{Single-node regime} regime: 1–8 accelerators connected by shared memory, where the binding constraint is the memory wall, the rate at which data moves from HBM (accelerator-local high-bandwidth memory) to compute units. At fleet scale, thousands of nodes coordinate across network fabrics and the bottleneck shifts toward bisection bandwidth\index{Bisection Bandwidth!fleet-scale bottleneck}, the aggregate capacity across a cut through the cluster network. For detailed guidance on reading paths, learning outcomes, prerequisites, and how to get the most from this textbook, the front-of-book About This Book chapter provides the orientation.
...
class AmdahlsPitfall:
    ...
    t_inf_new_ms_str = fmt_time(t_inf_new, 'millisecond', precision=0, commas=False)
    ...
    new_total_ms_str = fmt_time(t_total_new, 'millisecond', precision=0, commas=False)
